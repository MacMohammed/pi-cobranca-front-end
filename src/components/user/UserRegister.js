...
class UserRegister extends HTMLElement {
    constructor() {
        ...
        this.shadowRoot.addEventListener("submit", (event) => {
            ...
            const form = this.shadowRoot.querySelector("#cadastro_usuario");
    ...
    async setTemplate() {
        ...
        cargos.map(x => {
            options = `${options}<option value=${x.id}>${x.descricao.toUpperCase()}</option>`
        });
        ...
        <form id="cadastro_usuario">
            ...
            <ul class="flex-outer">
                ...
                <li>
                    ...
                    <input type="email" id="useremail" name="useremail" placeholder="Digite o e-mail" required>
                </li>
                <li>
    ...
    getCargos = async () => {
        const response = await fetch(urls.cargo.cargo, this.options);
        const data = await response.json();
        return data;
    }

    postUsuario = async (form) => {
            const data = Object.fromEntries(new FormData(form));
            const url = urls.usuario.cadastrar;
            ...
            .then(response => {
                if (!response.ok) {
                    throw new Error(`Não foi possível cadastrar o usuário ${data.username}.`);
                }
                return response.json();
            })
            .then((data) => {
                runModal(`O usuário ${data.nome} foi cadastrado com sucessos!`, "Usuário cadastrado...");
            })
            .catch(err => {
                runModal(err, "Cadastro negado...", );
            })
    }
